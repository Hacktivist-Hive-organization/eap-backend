...
class RequestService:
    ...
    def _validate_transition_and_role(
        self, request, new_status: Status, user: CurrentUser, comment: Optional[str]
    ):
        current_status = request.current_status
        allowed_transitions = REQUEST_STATE_CONFIG.get(current_status, {})

        # 403: role cannot act on current status at all
        roles_allowed_any = {
        ...
        # 403: if approver, must be assigned to request
        ...
        # 409: transition not allowed from this status
        transition_config = allowed_transitions.get(new_status)
        ...
        # 403: user's role is not allowed for this specific transition
        ...
        # 400: comment is required for this transition
